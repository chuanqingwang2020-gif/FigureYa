...
library(Boruta) # Boruta降维 # Boruta dimensionality reduction
library(ggplot2) # 画图 # Plotting
library(ggpubr) # 画图增强 # Plotting enhancement
library(pheatmap) # 热图 # Heatmap
...
## 用Boruta对ICI signature降维 Use Boruta to reduce the dimensionality of the ICI signature
...
### 原则上：一起构建预后签名 In principle: build prognostic signatures together
...
### 灵活处理：拆开A、B签名，分开降维 Flexible processing: Disassemble A and B signatures and reduce dimensionality separately
```{r}
# 提取A签名
# Extract A signature
set.seed(20201024)
...
## 用降维后的签名画热图 Draw a heatmap with the dimensionally reduced signature
```{r}
...
## 联合主成分分析第一主成分计算ICI得分 Combined principal component analysis (PCA) first principal component to calculate the ICI score
```{r}
...
## 画图对比ABC三类的ICI score Draw and compare the ICI scores of the three types of ABC
