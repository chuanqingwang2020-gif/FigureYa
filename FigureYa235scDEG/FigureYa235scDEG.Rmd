...
library(Seurat) # 单细胞数据处理 Single-cell data processing
library(patchwork) #拼图 Plotting
library(reshape2) #数据处理 Data processing
library(RColorBrewer) #颜色 Color
library(ggplot2) #绘图 Plotting
library(ggrepel)  # 标注文本 Annotation text
library(magrittr) #数据处理 Data processing
...
> The cutoff of the latest version of Seurat is different from before, Seurat 3.X.X and below use the natural logarithmic log, and Seurat 4.X.X uses log2, so if you use Seurat 4.X.X, you will end up with more points in the graph.
...
If you don't need to identify the cell types just to reproduce the diagram in the article, the following section can be done without running and determine the cell type based on the knowledge of the marker genes and biology that binds each cluster.
...
## Set cutoff to define significant/non-significant up/down marker gene
...
You can also manually pick out the genes you want to name based on output_pbmc.markers.csv files, such as genes involved in a pathway, and then name the file easy_input_label.csv
